		...
		<main>
			...
			<div class="card card-custom p-2 my-3 rounded-4 shadow-lg">
				<div class="card-body flex-column align-items-start">
					<h5 class="card-title text-primary">Kartu Tagihan</h5>
					<div class="card-text">
												...
												foreach ($kartutagihanbulanan as $mydata) {
													...
													for ($x = 1; $x <= 12; $x++) {
														if ($array1[$x] - $array2[$x] != 0) {
															...
															$colorClass = "class = 'text-dark'>";
														}
												?>
														<tr>
															<th scope="row" <?= ($cek == 0) ? $colorClass . $i : '' ?>></th>
															<td <?= $colorClass ?>><?= $mydata->nama . " - " . ShortNameOfMonth($x) ?></td>
															<td <?= $colorClass ?>><?= FormatRupiah($array1[$x]) ?></td>
															<td <?= $colorClass ?>><?= FormatRupiah($array1[$x] - $array2[$x]) ?></td>
														</tr>
												<?php
														$cek = $x;

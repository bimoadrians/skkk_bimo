...
if (!isset($_SESSION['logged_in']) || !$_SESSION['logged_in']) {
	...
	$query = "SELECT InsertIntoDB AS tanggal, Text AS isipesan FROM jbssms.outboxhistory WHERE Text LIKE '%" . $nama . "%' ORDER BY id DESC";
	$response = JalankanSQL("view", $query, 100);
					...
					<div class="card-text">
						<div class="col-auto">
							<?php if ($response) { ?>
									...
									<tr>
										  <th scope="row"><?=$i?></th>
										  <td><sup><b>Tanggal :</b> <?=$tanggal?><hr /><?=ConvertWAToHTML($mydata->isipesan) ?></sup></td>
									</tr>
								  <? } ?>
								  </tbody>
								</table>

...
$perintah		= 'view';
$limit			= 1;
$sqlUser		= "SELECT * FROM $dbSdm.user WHERE user_id = '" . $goUserId . "'";
$dataUser		= JalankanSQL($perintah, $sqlUser, $limit);
$userId			= $dataUser[0]->user_id;
$nama				= $dataUser[0]->nama;
$hp					= $dataUser[0]->hp;
$dataJs			= ['userId' => $userId, 'nama' => $nama, 'hp' => $hp];
$dataArrayJs = [$dataJs];
$jsonDataJs	= json_encode($dataArrayJs);
echo $jsonDataJs;
/*$today					= date('Y-m-d');
$sqlPresensi		= "SELECT * FROM $dbSdm.presensi WHERE user_id = '" . $userId . "' AND hari = '" . $today . "'";
...
/*ob_start();
echo "hasil : " . $userId . "-" . $nama . "-" . $hp . "\r\n";
$htmlStr = ob_get_contents();
ob_end_clean();
file_put_contents("hasil.txt", $htmlStr);*/
?>
